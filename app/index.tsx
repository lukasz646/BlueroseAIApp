...
export default function Dashboard() {
  ...
  const StatusPill = ({ state }: { state: BotState }) => {
    ...
    const bg =
      ...
      state === "stopped" ? "#475569" :
      state === "error" ? "#dc2626" : "#a78bfa";

    return (
      <View style={{ backgroundColor: bg, paddingHorizontal: 12, paddingVertical: 6, borderRadius: 999 }}>
        <Text style={{ color: "white", fontWeight: "700", letterSpacing: 1 }}>{label}</Text>
      </View>
    );
  };

  const Button = ({ title, onPress, variant = "primary" }: { title: string; onPress: () => void; variant?: "primary"|"danger" }) => (
    <TouchableOpacity
      onPress={onPress}
      disabled={loading}
      style={{
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: "#0b1020" }}>
      <View style={{ flex: 1, paddingHorizontal: 20, paddingTop: 24, gap: 18 }}>
        <Text style={{ color: "#e5e7eb", fontSize: 22, fontWeight: "800" }}>BlueRose AI – Dashboard</Text>

        <View style={{ backgroundColor: "#111634", padding: 18, borderRadius: 20, gap: 12 }}>
          <Text style={{ color: "#c7d2fe", fontWeight: "700" }}>Status bota</Text>
          <View style={{ flexDirection: "row", alignItems: "center", gap: 12 }}>
            <StatusPill state={botState} />
            {loading ? <ActivityIndicator /> : null}
          </View>
          <Text style={{ color: "#94a3b8" }}>
            Saldo: <Text style={{ color: "#e2e8f0", fontWeight: "700" }}>{balance ?? "—"}</Text>
          </Text>
        ...
        <TouchableOpacity
          onPress={loadStatus}
          disabled={loading}
          style={{ paddingVertical: 12, borderRadius: 12, alignItems: "center", backgroundColor: "#1f2937" }}
        >
          <Text style={{ color: "#cbd5e1", fontWeight: "700" }}>Odśwież status</Text>
        </TouchableOpacity>
